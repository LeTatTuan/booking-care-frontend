        ...
        if (inputData && inputData.length > 0) {
            ...
            if (type === 'PRICE') {
                inputData.map((item, index) => {
                    ...
                    object.label = language === LANGUAGES.VI ? item.valueVi : labelEn;
    ...
    handleChangeSelect = async (selectedOption) => {
        ...
        if (res && res.errCode === 0 && res.data && res.data.Markdown) {
            ...
            let addressClinic = '', nameClinic = '',
                note = '', priceId = '', paymentId = '', provinceId = '',
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        saveDetailDoctorData: (data) => dispatch(actions.saveDetailDoctorData(data))
    };
};
...
/*
fire vent: child -> parent (props)
parent -> child(ref)
emitter (event): tự động tác động 
send message: chat real time
client - receive
